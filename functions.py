...
def get_config(file = '/home/pes2ug19cs020/BigData/project/yah/Current_Config.json'):
	os.chdir(os.getcwd())
...
def configure(config):
	...
	#open(os.path.join(dlogpath,"datanode_log.log"),'w')
	for i in range(0,dnum):
		#open(os.path.join(dlogpath,"logger{}.log".format(i+1)),'w')
...
def load_dfs(file):
	...
	if f.lower() != 'yes':
		...
		for i in range(1,dnum+1):
			...
			for i in filelist:
				checkdict[i][1].append(i)
		os.chdir(ogpath)
		dictionary = get_status_datanodes()
		if checkdict == dictionary:
			return
	...
	for i in range(0,dnum):
		datanode_num[i+1]=[0,[]]
	logger.debug("{}".format(datanode_num))
	#whats left - other files also need to be formatted similarly + error checking 
	#whats left - "basic checks"	
...
def put(source,dest):
	...
	#whats left - proper error checking 
...
def mkdir(dir_name):
	...
	#check if something needs to be added
...
def cat(file):
	...
	with open(os.path.join(config['path_to_namenodes'],'namenode.txt'),'rb') as nfile:
		temp = pickle.load(nfile)
	for files in temp:
		#print(files[0],filename,files[1],directory)
...
def ls(dir_name):
	...
	config = get_config()
	fs_path = config['fs_path']
	path = os.path.join(fs_path,dir_name)
...
def rm(file):
	try:
		...
		with open(os.path.join(config['path_to_namenodes'],'namenode.txt'),'rb') as nfile:
			temp = pickle.load(nfile)
		for files in temp:
			#print(files[0],files[1],filename,directory)
			if files[0] == filename and files[1][:-1] == directory:
				for blocks in files[2]:
					for reps in blocks:
						os.remove(reps)
